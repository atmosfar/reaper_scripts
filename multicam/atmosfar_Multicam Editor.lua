...
function SetupMulticamTrack()
    ...
    if not tracks_selected then
        reaper.MB("No tracks selected", "Error: Multicam Editor Setup", 0)
        reaper.Undo_EndBlock("Setup multicam track", 0)
        return -1
    end

    local multicam_found = false
    local multicam_track = -1
    for i = 0, track_count - 1 do
        local track = reaper.GetTrack(proj, i)
    ...
    if not multicam_found then
        multicam_track = last_selected_index + 1
...
local function SetupVideoProcessor(tr)
  ...
  if file then
      ...
      if video_code and video_code ~= "" then
         ...
         reaper.MB("Error in SetupVideoProcessor(): File is empty or unreadable", 
          "Multicam Editor", 0)
         return false
      end
  else
     reaper.MB("Error in SetupVideoProcessor(): cannot open file " .. filename, 
      "Multicam Editor", 0)
     return false
  end
end
...
-- INITIALIZATION
local multicam_track = SetupMulticamTrack()
if multicam_track >= 0 then
  SetupVideoProcessor(multicam_track - reaper.CountSelectedTracks(0))
